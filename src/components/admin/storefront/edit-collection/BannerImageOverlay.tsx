...
export function EditBannerImageButton() {
  const { showOverlay } = useOverlayStore();

  const { pageName, overlayName } = useOverlayStore((state) => ({
    pageName: state.pages.editCollection.name,
    overlayName: state.pages.editCollection.overlays.bannerImage.name,
  }));
...
export default function BannerImageOverlay({
  collectionId,
  data,
}: {
  collectionId: string;
  data: { title: string; bannerImage: string | undefined };
}) {
  const HIDDEN = "HIDDEN";
  ...
  const [image, setImage] = useState(data.bannerImage);
  ...
  const { pageName, isOverlayVisible, overlayName } = useOverlayStore(
    (state) => ({
      ...
      overlayName: state.pages.editCollection.overlays.bannerImage.name,
      isOverlayVisible:
        state.pages.editCollection.overlays.bannerImage.isVisible,
    })
  );
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="pt-12 pb-10 px-4 flex flex-col gap-4">
                <div>
                  ...
                  <div className="w-full h-[308px] flex items-center justify-center overflow-hidden border rounded-t-[7px] mt-4">
                    {!image ? (
